...
const securityHeaders = [
  // XSS: only allow scripts from self + Cashfree SDK
  {
    ...
    value: [
      ...
      // (a blank white cup), and the only trace is a console warning. Same class of
      // silent failure as the Cashfree iframe and the S3 avatar PUT below/above.
      `connect-src 'self' blob: https://api.cashfree.com https://sandbox.cashfree.com https://nominatim.openstreetmap.org https://api.grabit365.com https://gradient-cafe-assets-676591241313.s3.ap-south-1.amazonaws.com${isDev ? ' http://localhost:8083' : ''}`,
