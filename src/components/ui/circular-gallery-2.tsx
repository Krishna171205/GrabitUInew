...
function autoBind(instance: object) {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== "constructor" && typeof (instance as Record<string, unknown>)[key] === "function") {
      (instance as Record<string, unknown>)[key] = (instance as Record<string, unknown>)[key];
    }
  });
}
...
class Media {
  ...
  createShader() {
    ...
    img.onload = () => {
      ...
      // Draw base image at original dimensions
      ctx.drawImage(img, 0, 0, img.naturalWidth, img.naturalHeight);

      // Bottom gradient for text legibility
      const grad = ctx.createLinearGradient(0, img.naturalHeight * 0.5, 0, img.naturalHeight);
      ...
      ctx.fillText(this.text, img.naturalWidth / 2, img.naturalHeight - Math.round(img.naturalHeight * 0.06));
  ...
  onResize(
    ...
    if (isMobile) {
      // Mobile: ~1.5 cards visible, card occupies ~62% of viewport width
      this.plane.scale.x = this.viewport.width * 0.62;
      this.plane.scale.y = this.plane.scale.x * 1.28;
    } else {
      this.plane.scale.y =
...
class App {
  ...
  createMedias(
    ...
    // Reduce bend on mobile so side cards don't arc off-screen
    const isMobile = this.screen.width < 640;
    const effectiveBend = isMobile ? Math.min(bend, 1.5) : bend;
...
const CircularGallery = ({
  ...
  useEffect(() => {
    ...
    const computedColor = computedStyle.color || "#1c1c1e";
