...
function measureScan() {
  ...
  bins.forEach((r, i) => {
    ...
    const y = -1 + (i + 0.5) / N * 2;
    console.log(y.toFixed(3).padStart(7), String(r.length).padStart(5),
      q(0.98).toFixed(3).padStart(8), q(0.5).toFixed(3).padStart(8));
  });
}
...
(async () => {
  ...
  for (const f of fs.readdirSync(OUT_DIR)) {
    if (/^grabbit-cup(\..+)?\.glb$/.test(f) && f !== name) fs.unlinkSync(path.join(OUT_DIR, f));
  }
